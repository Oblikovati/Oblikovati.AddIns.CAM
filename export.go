...
// Command oblikovati-cam is built as a c-shared library (.so/.dll/.dylib) and loaded
...
// on Activate it constructs the CAM engine (package bridge) bound to the host through
// the host-supplied callback. The host owns the model; this library owns the CAM
// pipeline (Job → operations → toolpath → G-code post).
package main
...
const addInID = "com.oblikovati.cam"
...
var (
	...
	engine   *bridge.Engine // active CAM engine, nil when inactive
)
