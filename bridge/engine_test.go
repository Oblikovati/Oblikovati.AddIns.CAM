...
func (h *recordingHost) Call(method string, payload []byte) ([]byte, error) {
	...
	case wire.MethodBodyList:
		if h.bodies != nil {
			return json.Marshal(wire.BodyListResult{Bodies: h.bodies})
		}
		return json.Marshal(wire.BodyListResult{Bodies: []wire.BodyInfo{{Index: 0, Name: "Body1", Solid: true, Visible: true}}})
...
func TestRibbonLayoutCoversEveryCommand(t *testing.T) {
	for _, c := range camCommands {
		if nonRibbonCommands[c.id] {
			continue // dialog-only command, intentionally off the ribbon
		}
...
func TestEveryCommandIsAnIconButton(t *testing.T) {
	for _, c := range camCommands {
		if nonRibbonCommands[c.id] {
			continue // dialog-only command, not a ribbon button
		}
