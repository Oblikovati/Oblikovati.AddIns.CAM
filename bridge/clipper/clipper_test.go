...
func ccwSquare() Path {
	return Path{{0, 0}, {10, 0}, {10, 10}, {0, 10}}
}
...
func TestAreaDegenerate(t *testing.T) {
	for _, p := range []Path{nil, {{0, 0}}, {{0, 0}, {10, 0}}} {
		if got := Area(p); got != 0 {
...
func TestPointInPolygon(t *testing.T) {
	...
	cases := []struct {
		...
		{"strictly inside", IntPoint{5, 5}, 1},
		{"outside to the right", IntPoint{15, 5}, 0},
		{"outside below", IntPoint{5, -5}, 0},
		{"on the left edge", IntPoint{0, 5}, -1},
		{"on the bottom edge", IntPoint{5, 0}, -1},
		{"on a corner vertex", IntPoint{10, 10}, -1},
	}
	for _, c := range cases {
...
func TestPointInPolygonDegenerate(t *testing.T) {
	if got := PointInPolygon(IntPoint{0, 0}, Path{{0, 0}, {1, 1}}); got != 0 {
...
func TestReversePaths(t *testing.T) {
	ps := Paths{{{0, 0}, {1, 0}}, {{0, 0}, {0, 1}}}
	ReversePaths(ps)
	if ps[0][0] != (IntPoint{1, 0}) || ps[1][0] != (IntPoint{0, 1}) {
...
func TestCleanPolygonRemovesCollinearVertex(t *testing.T) {
	...
	in := Path{{0, 0}, {50, 0}, {100, 0}, {100, 100}, {0, 100}}
...
func TestCleanPolygonRemovesNearDuplicate(t *testing.T) {
	...
	in := Path{{0, 0}, {100, 0}, {100, 1}, {100, 100}, {0, 100}}
	out := CleanPolygon(in, 2)
	if len(out) != 4 {
...
func TestCleanPolygonCollapsesToEmpty(t *testing.T) {
	...
	if out := CleanPolygon(Path{{0, 0}, {1, 0}, {2, 0}}, 5); len(out) != 0 {
...
func TestCleanPolygons(t *testing.T) {
	out := CleanPolygons(Paths{{{0, 0}, {50, 0}, {100, 0}, {100, 100}, {0, 100}}}, 2)
