...
func TestCoolantBlocks(t *testing.T) {
	...
	names := commandNames(PostObjects(res)[0].Path.Commands)
...
func TestNoCoolantNoBlocks(t *testing.T) {
	...
	for _, n := range commandNames(PostObjects(res)[0].Path.Commands) {
		if n == "M7" || n == "M8" || n == "M9" {
...
func TestCoolantParamEdit(t *testing.T) {
	...
	if !strings.Contains(strings.Join(commandNames(PostObjects(res)[0].Path.Commands), ","), "M7") {
