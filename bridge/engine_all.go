...
func (e *Engine) surfaceOp(job *Job, stock Stock, tris []ocl.Triangle, cut cutSettings) (*SurfaceOp, error) {
	rows, err := e.dropCutterRows(job, stock, tris, cut, scanLines)
	if err != nil {
		return nil, err
	}
	ball := indexForShape(job.Tools, "ballend")
	stepOver := passSpacing(cut, job.Tools[ball].Tool.Diameter, surfStepOver)
	env := e.millEnvelope("Surface", stock)
	env.ToolController = ball
	return &SurfaceOp{OpBase: env, StepOver: stepOver, Sampling: surfSampling, Zigzag: true, Rows: rows}, nil
}
