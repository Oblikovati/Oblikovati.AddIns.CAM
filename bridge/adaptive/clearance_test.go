...
func TestIsAllowedToCutThroughShortMove(t *testing.T) {
	...
	ok, err := rp.isAllowedToCutThrough(res.toolPos, near, 1.5, false)
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
...
func TestIsAllowedToCutThroughRejectsOutsideRegion(t *testing.T) {
	...
	ok, err := rp.isAllowedToCutThrough(res.toolPos, outside, 1.5, false)
	if err != nil {
		t.Fatal(err)
	}
	if ok {
...
func TestIsAllowedToCutThroughRejectsOverEngagement(t *testing.T) {
	...
	ok, err := rp.isAllowedToCutThrough(res.entryPoint, deep, 1.5, false)
	if err != nil {
		t.Fatal(err)
	}
	if ok {
